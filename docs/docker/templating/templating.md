...
We can create a template in two ways:
...
Now its time for the ports. We press the same `Add another Path, Port, Variable or Device` link. This time we need to change the config type to `Port`.
...
* ==WebUI== Which container-port a webui might be on. e.g. `http://[IP]:[PORT:5800]` Unraid will translate this string to the IP of the server, and the host-port set for container-port 5800
